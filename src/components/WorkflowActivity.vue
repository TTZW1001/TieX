...
const entries = computed<ActivityEntry[]>(() => {
  ...
  for (const session of commandSessionList.value) {
    list.push({
...
const pendingCount = computed(() => entries.value.filter((entry) => entry.status === 'running' || entry.status === 'waiting').length)
...
function getCommandSession(sessionId: string) {
...
<template>
  <section v-if="entries.length" class="activity">
    <div class="activity-head">
      <div class="activity-labels">
        <span class="activity-caption">活动</span>
        <span class="activity-count">{{ entries.length }} 条</span>
      </div>
      <div v-if="pendingCount" class="activity-live">
        <span class="activity-live-dot" />
        <span>{{ pendingCount }} 项处理中</span>
      </div>
    </div>
...
.activity-row:last-child .activity-rail {
  display: none;
}
